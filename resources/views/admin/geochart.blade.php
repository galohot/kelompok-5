          ...
          <div class="col-sm-12">
            <div class="table-responsive">
              <table id="dataTable" class="table card-table table-vcenter text-nowrap datatable dataTable" role="grid">
                ...
                <tbody> @foreach ($data as $countryCode => $gdp) @php $countryName = \App\Models\CountryMaster::where('CountryCode', $countryCode)->value('Country'); $countryLowerCase = \App\Models\CountryMaster::where('CountryCode', $countryCode)->value('CCLower'); @endphp <tr>
                    <td>
                      <span class="flag flag-country-{{ $countryLowerCase }}"></span>
                    </td>
                    <td class="text-wrap">
                      <a href="/admin/countryprofile/{{ $countryCode }}">{{ $countryName }}</a>
                    </td>
                    <td>{{ $gdp }} %</td>
                  </tr> @endforeach </tbody>
              </table>
